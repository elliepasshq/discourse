    ...
    class << self
      def check(user, capability, trace_id: nil)
        ...
        if durable && location
          ...
          return decision_from_snapshots(
            state,
            capability,
            durable,
            location,
        ...
        response =
          Elliepass::ApiClient.authorize_member(
            user,
            capability,
            trace_id: trace_id,
          )
        ...
        durable_ttl =
          SiteSetting
            .elliepass_durable_cache_hours
            .hours

        location_ttl =
          SiteSetting
            .elliepass_location_cache_hours
            .hours

        if durable.is_a?(Hash)
        ...
        result = {
          ...
          policy_applies:
            response["policy_applies"] == true,
          reason: response["reason"],

          durable_required:
            durable_required?(state),

          durable_satisfied:
            durable.is_a?(Hash) &&
              durable["satisfied"] == true,
      ...
      def guarded?(state, capability)
        state.guarded_actions[
          capability.to_s
        ] == true
      end

      def durable_cache_key(
        user,
        policy_version
      )
        [
      ...
      def durable_required?(state)
        ...
        %w[
          ...
          needs_identity
          needs_age
          needs_id_verified_location
        ].any? do |key|
          requirements[key] == true ||
            requirements[key.to_sym] == true
        end
      end
      
      def decision_from_snapshots(
        state,
        capability,
        durable,
        location
